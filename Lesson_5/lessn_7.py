...
try:
    with open("text_lesson_7.txt", encoding='utf-8') as fl:
        lines = fl.readlines()

    for line in lines:
        name, firm, proceeds, expenses = line.split()
        result[0][name] = int(proceeds) - int(expenses)

    result[1]['average_profit'] = round(
        sum(
            profit for _, profit in result[0].items() if profit > 0
        ) / len([profit for _, profit in result[0].items() if profit > 0])
    )
    print(result)
    with open("text_lesson_7.json", "w", encoding="UTF-8") as fl:
